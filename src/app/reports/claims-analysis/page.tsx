...
export default async function ClaimsAnalysisPage({ searchParams }: ClaimsAnalysisPageProps) {
  ...
  const summaryRows: ClaimsStatusSummaryRow[] = statuses.map((status) => {
    ...
    return {
      ...
      reduction: statusRows.reduce((sum, row) => sum + Math.max(0, row.reduction), 0),
  ...
  const ledgerRows: ClaimsAnalysisLedgerRow[] = rows.map((row) => ({
    ...
    reduction: Math.max(0, row.reduction), collectionPct: row.collectionPct,
  }));

  return <AppShell>
    <PageHeader title="Claims Analysis" subtitle="Claim volume, status mix, and reductions across all dimensions" />
    ...
    <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7" aria-label="Claims analysis key performance indicators">
      ...
      <StatCard label="Underpayment" value={kpis.underpayment} /><StatCard label="Phantom" value={kpis.phantom} />

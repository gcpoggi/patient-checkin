...
export default function Home() {
  ...
  const statuses = ["paid_full", "unpaid", "underpayment", "denied"] as const;
  const statusCounts = { paid_full: kpis.paidFull, unpaid: kpis.unpaid, underpayment: kpis.underpayment, denied: kpis.denied };
  const claimsAtRisk = kpis.unpaidAmount + kpis.deniedAmount + kpis.phantomAmount;
  ...
  return (
    <AppShell>
      ...
      <section className="mt-6 flex flex-col gap-4 rounded-xl border border-mist-200 bg-white px-5 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between" aria-labelledby="month-end-status">
        ...
        <div className="flex flex-wrap gap-2">{statuses.map((status) => <Link key={status} href={status === "paid_full" ? "/claims?month=2026-01&office=kendall" : `/claims?month=2026-01&office=kendall&status=${status}`} className="inline-flex items-center gap-2 rounded-full border border-mist-200 bg-mist-50 py-1 pl-1 pr-3 transition hover:border-teal-300 hover:bg-mist-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-hpp"><StatusBadge status={status} /><span className="font-mono text-xs font-semibold tabular-nums text-navy">{statusCounts[status]}</span></Link>)}</div>
      </section>
    </AppShell>
  );
}

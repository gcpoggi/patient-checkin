  ...
  return (
    <AppShell>
      ...
      <section className="mt-6 overflow-hidden rounded-xl border border-mist-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-mist-100 text-left text-xs uppercase tracking-wider text-navy"><tr><th className="px-4 py-2">Claim</th><th className="px-4 py-2">Patient</th><th className="px-4 py-2">CPT</th><th className="px-4 py-2 text-right">Billed</th><th className="px-4 py-2 text-right">Paid</th><th className="px-4 py-2 text-right">Reduction</th></tr></thead><tbody className="divide-y divide-mist-200">{claims.map((claim, index) => <tr key={contestation.claimIds[index]}><td className="px-4 py-3 font-mono">{contestation.claimIds[index]}</td><td className="px-4 py-3">{claim?.patientName ?? "Claim unavailable"}</td><td className="px-4 py-3">{claim?.cptCode ?? "—"}</td><td className="px-4 py-3 text-right font-mono">{claim ? money.format(claim.billedAmount) : "—"}</td><td className="px-4 py-3 text-right font-mono">{claim ? money.format(claim.paidAmount) : "—"}</td><td className="px-4 py-3 text-right font-mono">{claim ? money.format(Math.max(0, allowedAmountForClaim(claim) - claim.paidAmount)) : "—"}</td></tr>)}</tbody></table></div>
      </section>

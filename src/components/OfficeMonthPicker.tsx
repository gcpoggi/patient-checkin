...
export function OfficeMonthPicker({ current, month }: { current: OfficeId; month: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 rounded-xl border border-mist-200 bg-white p-3 shadow-sm">
      <div className="flex rounded-lg bg-mist-100 p-1" aria-label="Office">
        {(["kendall", "ponce"] as const).map((office) => (
          <button key={office} type="button" onClick={() => update("office", office)}
            className={`rounded-md px-4 py-2 text-sm font-semibold capitalize transition ${current === office ? "bg-teal-500 text-white shadow-sm" : "text-slate-600 hover:text-navy"}`}
      ...
      <span className="ml-auto font-mono text-sm font-semibold tabular-nums text-teal-700">2026</span>
    </div>
  );
}

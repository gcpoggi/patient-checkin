...
export function CollectionsBar({ rows }: { rows: ReconciledClaimRow[] }) {
  const buckets = Array.from({ length: 5 }, (_, index) => ({ week: `Week ${index + 1}`, Billed: 0, Allowed: 0, Collected: 0 }));
  for (const row of rows) {
    const bucket = Math.min(4, Math.floor((Number(row.dateOfService.slice(8, 10)) - 1) / 7));
    buckets[bucket].Billed += row.billedAmount;
    buckets[bucket].Allowed += row.allowedAmount;
    buckets[bucket].Collected += row.paidAmount;
  }
  const data = buckets.filter((bucket, index) => index < 4 || bucket.Billed || bucket.Allowed || bucket.Collected);
  const hasActivity = rows.some((row) => row.billedAmount > 0 || row.allowedAmount > 0 || row.paidAmount > 0);
  ...
  return (
    <section className="rounded-xl border border-mist-200 bg-white p-5 shadow-sm">
      ...
      <h2 className="mt-1 font-display text-xl font-semibold text-navy">Billed vs allowed vs collected</h2>
      {!hasActivity ? (
      ...
      ) : <div className="mt-5 h-72" aria-label="Weekly billed, allowed, and collected amounts">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
            ...
            <Bar dataKey="Allowed" fill={CHART_COLORS.navy} radius={[4, 4, 0, 0]} />

...
function visitTypeLabel(value: ServiceTransaction["eventType"]): string {
  return value === "doctor" || value === "followup"
    ? DOCTOR_VISIT_TYPE_LABELS[value]
    : String(value);
}
...
const columns: ExcelColumn<ServiceTransaction>[] = [
  ...
  { key: "date", header: "Date", width: 100 },
  {
    ...
    header: "Visit Type",
    filter: "select",
    filterOptions: ["doctor", "followup"],
    render: (_, row) => visitTypeLabel(row.eventType),
    exportValue: (row) => visitTypeLabel(row.eventType),
  },
  ...
  { key: "paidAmount", header: "Paid", align: "right", render: money },
  {
    ...
    render: (_, row) => (
      <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusClasses[row.billingStatus]}`}>
        {statusLabels[row.billingStatus]}
      </span>
    ),
...
export function DoctorVisitsTable({ rows, exportFilename }: DoctorVisitsTableProps) {
  return (
    <ExcelTable
      columns={columns}
      rows={rows}
      ...
      title="Doctor visit transactions"
      empty="No doctor visits found for this office and month."
    />
  );
}

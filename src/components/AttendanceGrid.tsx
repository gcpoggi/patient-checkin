  ...
  return (
    <section className="overflow-hidden rounded-xl border border-mist-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full table-fixed border-separate border-spacing-0 text-center font-mono text-[11px] tabular-nums">
          <thead><tr><th className="sticky left-0 z-20 w-20 border-b border-r border-mist-200 bg-navy px-2 py-3 text-left font-sans text-[11px] uppercase tracking-wider text-white">Time</th>{attendance.dates.map((date) => <th key={date} className="border-b border-r border-mist-200 bg-navy px-1 py-3 text-white">{Number(date.slice(5,7))}/{Number(date.slice(8))}</th>)}<th className="w-14 border-b bg-teal-700 px-1 py-3 text-white">TOTAL</th></tr></thead>
          <tbody>
            {attendance.slots.map((slot, index) => <SlotRows key={slot} slot={slot} attendance={attendance} lunchAfter={index === 4} />)}
            {footerRows.map(([label, key]) => <tr key={label} className={label === "TOTAL" ? "font-bold" : ""}><th className="sticky left-0 z-10 border-r border-t border-mist-200 bg-mist-100 px-2 py-2 text-left font-sans text-[10px] tracking-wide text-navy">{label}</th>{attendance.dates.map((date) => <td key={date} className="border-r border-t border-mist-200 bg-mist-50 px-1 py-2 text-navy">{attendance.dayTotals[date][key]}</td>)}<td className="border-t border-mist-200 bg-navy px-1 py-2 font-bold text-white">{attendance.monthTotals[key]}</td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
...
function SlotRows({ slot, attendance, lunchAfter }: { slot: TimeSlot; attendance: AttendanceMonth; lunchAfter: boolean }) {
  ...
  return <>
    <tr><th className="sticky left-0 z-10 border-b border-r border-mist-200 bg-white px-2 py-2 text-left font-sans font-semibold text-navy">{labels[slot]}</th>{attendance.dates.map((date) => { const value = attendance.grid[slot][date].attended; return <td key={date} className={`border-b border-r border-mist-200 px-1 py-2 ${heat[Math.min(5, value)]}`}>{value}</td>; })}<td className="border-b border-mist-200 bg-mist-100 px-1 py-2 font-bold text-navy">{slotTotal}</td></tr>
    <tr><th className="sticky left-0 z-10 border-b border-r border-mist-200 bg-missing-bg px-2 py-1 text-left font-sans text-[10px] font-bold tracking-widest text-missing">EVAL</th>{attendance.dates.map((date) => <td key={date} className="border-b border-r border-mist-200 bg-missing-bg/50 px-1 py-1 text-[10px] text-missing">{attendance.grid[slot][date].evals}</td>)}<td className="border-b border-mist-200 bg-missing-bg px-1 py-1 text-missing">{evalTotal}</td></tr>
    {lunchAfter ? <tr><th className="sticky left-0 z-10 border-b border-r border-mist-200 bg-slate-200 px-2 py-2 text-left font-sans font-semibold text-slate-600">12:00 PM</th><td colSpan={attendance.dates.length + 1} className="border-b border-mist-200 bg-slate-200 px-3 py-2 font-sans font-semibold tracking-[0.3em] text-slate-500">LUNCH</td></tr> : null}
  </>;
}
